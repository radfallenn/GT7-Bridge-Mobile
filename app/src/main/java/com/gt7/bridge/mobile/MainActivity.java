...
public class MainActivity extends Activity {
    ...
    public void onCreate(Bundle b) {
        ...
        TextView title = new TextView(this); title.setText("GT7 Bridge Mobile v1.4.1"); title.setTextSize(22); title.setTextColor(Color.BLACK); root.addView(title);
    ...
    private WebResourceResponse localBridgeResponse(String url){
        String body;
        synchronized(t){ body = url.contains("/api/status") ? t.status() : (url.contains("/api/map") ? t.map() : (url.contains("/api/entry") ? t.entryJson() : t.json())); }
    ...
    private void injectBridgeHints(){
        String js = "try{"+
            ...
            "window.dispatchEvent(new CustomEvent('gt7-bridge-mobile-ready',{detail:{url:'http://127.0.0.1:8787/api/fields',entryUrl:'http://127.0.0.1:8787/api/entry',mode:'mobile-apk-push'}}));"+
    ...
    private void pushTelemetryToWebView(){
        ...
        String js = "try{"+
            ...
            "try{localStorage.setItem('gt7LastMobileTelemetry',JSON.stringify(d));localStorage.setItem('gt7LastMobileEntry',JSON.stringify(e));}catch(x){}"+
