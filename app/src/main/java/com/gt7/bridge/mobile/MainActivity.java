...
public class MainActivity extends Activity {
    ...
    private void parse(byte[] d, int packetSize, Telemetry x){
        x.connected = true; x.updatedAt = System.currentTimeMillis(); x.lastPacketSize = packetSize; x.packetVersion = packetName(packetSize);
        ...
        int lap = b.getShort(0x74); int best = b.getInt(0x78); int last = b.getInt(0x7C); int flags = b.getShort(0x8E)&0xffff;
        int gearByte = d[0x90]&255, thr = d[0x91]&255, br = d[0x92]&255;
        boolean ok = finite(speed) && speed >= 0 && speed < 800 && finite(rpm) && rpm >= 0 && rpm < 30000 && lap > -20 && lap < 1000;
        x.valid = ok;
        if(!ok){ x.warning="Pacote recebido, mas valores ignorados por validacao."; return; }
        if(x.startMs==0) x.startMs = System.currentTimeMillis();
        x.warning=""; x.speed=speed; x.rpm=rpm; x.fuel=Math.max(0,fuel); x.fuelCap=cap; x.fuelPct=cap>0.1f?clamp((fuel/cap)*100f,0,100):-1;
        x.gear=gearByte&15; x.gearLabel=x.gear==0?"N":x.gear==15?"R":String.valueOf(x.gear);
        x.throttle=Math.max(0,Math.min(100,Math.round(thr/255f*100f))); x.brake=Math.max(0,Math.min(100,Math.round(br/255f*100f)));
        if(lap>=0)x.laps=lap; x.best=best>0?best:-1; x.last=last>0?last:-1; x.tcs=(flags&(1<<11))!=0; x.raceMs=System.currentTimeMillis()-x.startMs;
        x.x=f(b,0x04); x.y=f(b,0x08); x.z=f(b,0x0C); x.map.add(new float[]{x.x,x.y,x.z}); if(x.map.size()>1500)x.map.remove(0);
        if(d.length>=368){ int cur=b.getInt(0x15C); if(cur>0)x.current=cur; x.surface=ascii(d,0x158,4); }
        if(x.prevFuel>=0 && x.fuel>x.prevFuel+5 && x.speed<30)x.pits++; x.prevFuel=x.fuel;
    }
    ...
    private void httpServer(){
        try(ServerSocket server = new ServerSocket(8787)){
            while(true){ Socket c=server.accept(); BufferedReader br=new BufferedReader(new InputStreamReader(c.getInputStream())); String line=br.readLine(); String path="/api/fields"; if(line!=null){String[] p=line.split(" "); if(p.length>1)path=p[1];}
                String body; synchronized(t){ body=path.startsWith("/api/status")?t.status():path.startsWith("/api/map")?t.map():t.json(); }
                byte[] bytes=body.getBytes("UTF-8"); OutputStream out=c.getOutputStream(); String h="HTTP/1.1 200 OK\r\nContent-Type: application/json; charset=utf-8\r\nAccess-Control-Allow-Origin: *\r\nConnection: close\r\nContent-Length: "+bytes.length+"\r\n\r\n"; out.write(h.getBytes("UTF-8")); out.write(bytes); out.flush(); c.close(); }
        }catch(Exception ignored){}
    }

    static class Telemetry{
        boolean connected=false, valid=false, tcs=false; long updatedAt=0,startMs=0,raceMs=0; int lastPacketSize=0,gear=0,throttle=0,brake=0,laps=0,pits=0,best=-1,last=-1,current=-1; float speed=0,rpm=0,fuel=0,fuelCap=0,fuelPct=-1,x=0,y=0,z=0,prevFuel=-1; String gearLabel="N",packetVersion="?",warning="",surface="",rain="desconhecido"; ArrayList<float[]> map=new ArrayList<>();
        void reset(){connected=false;valid=false;updatedAt=0;startMs=0;raceMs=0;lastPacketSize=0;gear=0;gearLabel="N";throttle=0;brake=0;laps=0;pits=0;best=-1;last=-1;current=-1;speed=0;rpm=0;fuel=0;fuelCap=0;fuelPct=-1;x=0;y=0;z=0;prevFuel=-1;packetVersion="?";warning="";surface="";map.clear();}
        String json(){return "{\"connected\":"+connected+",\"decodeOk\":"+valid+",\"status\":\""+(valid?"ok":"aguardando_dados_validos")+"\",\"velocidade\":"+r(speed)+",\"rpm\":"+Math.round(rpm)+",\"marcha\":\""+gearLabel+"\",\"marchaNumero\":"+gear+",\"acelerador\":"+throttle+",\"freio\":"+brake+",\"combustivel\":"+r(fuel)+",\"combustivelPorcentagem\":"+r(fuelPct)+",\"melhorVolta\":\""+fmt(best)+"\",\"tempoTotalCorrida\":\""+fmt((int)raceMs)+"\",\"voltasCompletadas\":"+laps+",\"chuva\":\""+rain+"\",\"controleTracao\":\""+(tcs?"ativo":"inativo")+"\",\"numeroParadas\":"+pits+",\"packetVersion\":\""+packetVersion+"\",\"lastPacketSize\":"+lastPacketSize+",\"surfaceType\":\""+surface+"\",\"position\":{\"x\":"+r(x)+",\"y\":"+r(y)+",\"z\":"+r(z)+"},\"warning\":\""+warning+"\"}";}
        String status(){return "{\"connected\":"+connected+",\"decodeOk\":"+valid+",\"packetVersion\":\""+packetVersion+"\",\"lastPacketSize\":"+lastPacketSize+",\"warning\":\""+warning+"\"}";}
        String map(){StringBuilder s=new StringBuilder("{\"points\":["); for(int i=0;i<map.size();i++){float[] p=map.get(i); if(i>0)s.append(','); s.append("{\"x\":").append(r(p[0])).append(",\"y\":").append(r(p[1])).append(",\"z\":").append(r(p[2])).append('}');} return s.append("]}").toString();}
        String toPretty(){return "Status: "+(valid?"dados válidos":"aguardando dados válidos")+"\nPacote: "+packetVersion+" / "+lastPacketSize+"\nVelocidade: "+(int)speed+" km/h\nRPM: "+Math.round(rpm)+"\nMarcha: "+gearLabel+"\nAcelerador: "+throttle+"%\nFreio: "+brake+"%\nCombustível: "+r(fuel)+" L\nVoltas: "+laps+(warning.length()>0?"\nAviso: "+warning:"");}
        String r(float v){return v<0?"null":String.format(Locale.US,"%.2f",v);} String fmt(int ms){if(ms<=0)return "--";return String.format(Locale.US,"%d:%02d.%03d",ms/60000,(ms%60000)/1000,ms%1000);} }
}
